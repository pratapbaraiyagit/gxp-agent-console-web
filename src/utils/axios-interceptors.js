...
const setupInterceptors = ({ navigate, dispatch }) => {
  const startAutoRefreshTimer = () => {
    ...
    const token = getSessionItem("TokenAgentConsole");
    if (!token) {
      return;
    }
  ...
  const handleLogout = () => {
    ...
    removeSessionItem("UserSessionAgentConsole");
    removeSessionItem("TokenAgentConsole");
    removeSessionItem("RefreshAgentConsole");
    ...
    async (error) => {
      if (error.response?.status === 403 || error.response?.status === 401) {
        try {
          ...
          const newToken = getSessionItem("TokenAgentConsole");
