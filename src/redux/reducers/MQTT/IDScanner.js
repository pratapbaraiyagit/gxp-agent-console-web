...
import { base64ToOCR, idScannerMQTT } from "../../../utils/apiEndPoint";
...
const initialState = {
  ...
  // base64toocr
  base64ToOCRLoading: false,
  base64ToOCRDetails: {},
  isBase64ToOCRUpdate: false,
...
export const base64ToOCRData = createAsyncThunk(
  "id_scanner/base64toocr",
  (payload, { dispatch }) => {
    return new Promise((resolve, reject) => {
      axios
        .post(base64ToOCR, payload)
        .then((res) => {
          if (res.data.status) {
            resolve(res.data.data);
          } else {
    ...
    return new Promise((resolve, reject) => {
      axios
        ...
        .then((res) => {
          if (res.data.status) {
            // notification(
            //   res.data.message || "Successfully disconnected",
            //   "success"
            // );
            resolve(res.data.data);
          } else {
    ...
    return new Promise((resolve, reject) => {
      axios
        ...
        .then((res) => {
          if (res.data.status) {
            // notification(
            //   res.data.message || "Calibration successful",
            //   "success"
            // );
            resolve(res.data.data);
          } else {
...
export const idScannerSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // base64ToOCRData cases
      .addCase(base64ToOCRData.pending, (state) => {
        state.base64ToOCRLoading = true;
        state.isBase64ToOCRUpdate = false;
      })
      .addCase(base64ToOCRData.rejected, (state) => {
        state.base64ToOCRLoading = false;
        state.isBase64ToOCRUpdate = false;
      })
      .addCase(base64ToOCRData.fulfilled, (state, action) => {
        state.base64ToOCRLoading = false;
        state.isBase64ToOCRUpdate = true;
        state.base64ToOCRDetails = action.payload;
      })
